...
- **Upstream version in release notes:** `publish.yml` has a repo-owned `Get upstream provider version` step that resolves the `upstream` submodule commit to its release tag and prepends a "Wraps upstream terraform-provider-formal vX.Y.Z" line to the GitHub release body. Do not read the version from `provider/go.mod`: its requirement can remain stale because the local `replace` directive makes builds use the submodule. Running `make ci-mgmt` overwrites `publish.yml`, so re-apply this step (and the `body:` reference to its output) afterward — same class of manual patch as the `GITHUB_TOKEN` one above.
