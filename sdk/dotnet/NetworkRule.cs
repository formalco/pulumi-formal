// *** WARNING: this file was generated by pulumi-language-dotnet. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace Formal.Pulumi
{
    /// <summary>
    /// Creating a Network Rule in Formal.
    /// </summary>
    [PulumiResourceType("formal:index/networkRule:NetworkRule")]
    public partial class NetworkRule : global::Pulumi.CustomResource
    {
        /// <summary>
        /// The CEL expression describing how this Network Rule matches and routes traffic.
        /// </summary>
        [Output("celExpression")]
        public Output<string> CelExpression { get; private set; } = null!;

        /// <summary>
        /// When the Network Rule was created.
        /// </summary>
        [Output("createdAt")]
        public Output<string> CreatedAt { get; private set; } = null!;

        /// <summary>
        /// Network Rule description.
        /// </summary>
        [Output("description")]
        public Output<string?> Description { get; private set; } = null!;

        /// <summary>
        /// Network Rule name.
        /// </summary>
        [Output("name")]
        public Output<string> Name { get; private set; } = null!;

        /// <summary>
        /// Defines the current status of the Network Rule. It can be one of the following: 'draft' or 'active'.
        /// </summary>
        [Output("status")]
        public Output<string?> Status { get; private set; } = null!;

        /// <summary>
        /// If set to true, this Network Rule cannot be deleted.
        /// </summary>
        [Output("terminationProtection")]
        public Output<bool?> TerminationProtection { get; private set; } = null!;

        /// <summary>
        /// Last update time.
        /// </summary>
        [Output("updatedAt")]
        public Output<string> UpdatedAt { get; private set; } = null!;


        /// <summary>
        /// Create a NetworkRule resource with the given unique name, arguments, and options.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resource</param>
        /// <param name="args">The arguments used to populate this resource's properties</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public NetworkRule(string name, NetworkRuleArgs args, CustomResourceOptions? options = null)
            : base("formal:index/networkRule:NetworkRule", name, args ?? new NetworkRuleArgs(), MakeResourceOptions(options, ""))
        {
        }

        private NetworkRule(string name, Input<string> id, NetworkRuleState? state = null, CustomResourceOptions? options = null)
            : base("formal:index/networkRule:NetworkRule", name, state, MakeResourceOptions(options, id))
        {
        }

        private static CustomResourceOptions MakeResourceOptions(CustomResourceOptions? options, Input<string>? id)
        {
            var defaultOptions = new CustomResourceOptions
            {
                Version = Utilities.Version,
                PluginDownloadURL = "github://api.github.com/formalco",
            };
            var merged = CustomResourceOptions.Merge(defaultOptions, options);
            // Override the ID if one was specified for consistency with other language SDKs.
            merged.Id = id ?? merged.Id;
            return merged;
        }
        /// <summary>
        /// Get an existing NetworkRule resource's state with the given name, ID, and optional extra
        /// properties used to qualify the lookup.
        /// </summary>
        ///
        /// <param name="name">The unique name of the resulting resource.</param>
        /// <param name="id">The unique provider ID of the resource to lookup.</param>
        /// <param name="state">Any extra arguments used during the lookup.</param>
        /// <param name="options">A bag of options that control this resource's behavior</param>
        public static NetworkRule Get(string name, Input<string> id, NetworkRuleState? state = null, CustomResourceOptions? options = null)
        {
            return new NetworkRule(name, id, state, options);
        }
    }

    public sealed class NetworkRuleArgs : global::Pulumi.ResourceArgs
    {
        /// <summary>
        /// The CEL expression describing how this Network Rule matches and routes traffic.
        /// </summary>
        [Input("celExpression", required: true)]
        public Input<string> CelExpression { get; set; } = null!;

        /// <summary>
        /// Network Rule description.
        /// </summary>
        [Input("description")]
        public Input<string>? Description { get; set; }

        /// <summary>
        /// Network Rule name.
        /// </summary>
        [Input("name")]
        public Input<string>? Name { get; set; }

        /// <summary>
        /// Defines the current status of the Network Rule. It can be one of the following: 'draft' or 'active'.
        /// </summary>
        [Input("status")]
        public Input<string>? Status { get; set; }

        /// <summary>
        /// If set to true, this Network Rule cannot be deleted.
        /// </summary>
        [Input("terminationProtection")]
        public Input<bool>? TerminationProtection { get; set; }

        public NetworkRuleArgs()
        {
        }
        public static new NetworkRuleArgs Empty => new NetworkRuleArgs();
    }

    public sealed class NetworkRuleState : global::Pulumi.ResourceArgs
    {
        /// <summary>
        /// The CEL expression describing how this Network Rule matches and routes traffic.
        /// </summary>
        [Input("celExpression")]
        public Input<string>? CelExpression { get; set; }

        /// <summary>
        /// When the Network Rule was created.
        /// </summary>
        [Input("createdAt")]
        public Input<string>? CreatedAt { get; set; }

        /// <summary>
        /// Network Rule description.
        /// </summary>
        [Input("description")]
        public Input<string>? Description { get; set; }

        /// <summary>
        /// Network Rule name.
        /// </summary>
        [Input("name")]
        public Input<string>? Name { get; set; }

        /// <summary>
        /// Defines the current status of the Network Rule. It can be one of the following: 'draft' or 'active'.
        /// </summary>
        [Input("status")]
        public Input<string>? Status { get; set; }

        /// <summary>
        /// If set to true, this Network Rule cannot be deleted.
        /// </summary>
        [Input("terminationProtection")]
        public Input<bool>? TerminationProtection { get; set; }

        /// <summary>
        /// Last update time.
        /// </summary>
        [Input("updatedAt")]
        public Input<string>? UpdatedAt { get; set; }

        public NetworkRuleState()
        {
        }
        public static new NetworkRuleState Empty => new NetworkRuleState();
    }
}
